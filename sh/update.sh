...
# Get file from repo
ERR=$(curl -s -w "%{http_code}" -o "${CACHE_DIR}/${CACHE_DEB_FILE}" "https://gitlab.com/cbuild/cbuild/-/raw/master/packages/ppa/ubuntu/libcbuild-${LATEST_VERSION}.deb")
if [[ "$ERR" -ne "200" ]]; then
    echo "Error: File with version ${LATEST_VERSION} is not there";
    echo "Url used: \"https://gitlab.com/cbuild/cbuild/-/raw/master/packages/ppa/ubuntu/libcbuild-${LATEST_VERSION}.deb\""
    rm -rf $CACHE_DIR;
    exit 0;
fi
# Install package
