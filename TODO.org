...
*** TODO cbuild_fs_normalize broken on utf8 path
...
*** TODO Command.h - allow to limit number of async threads in 'cbuild_cmd_run'
...
*** TODO Command.h - better redirects
Alow to redirect directly to fine, not fd (will be autoclosed)
Maybe better piping?
*** DONE Command.h - allow to print command using single opt
*** TODO function to get time and nanosecond time in platform-independant way
*** TODO abstract filesize getting
*** TODO add needs_rebuild to abstract compare_mtime
*** TODO Proc.h - add ability to get number of CPU cores. By default command should scale to this+1 for async runs
* Thinking place
...
** TODO Command.h process waiting
...
or just busy-loop on process list with ~waitpid(pid, &status NOHANG)~ and ~nnanosleep~ to not fully busy-loop. This is actiully the best way (fastest/simplest)
