int main(void) {
	...
	TEST_ASSERT_EQ(r2, 0,
		"Comparison with same string builder failed"TEST_EXPECT_MSG(d), 0, r2);
	int r3 = cbuild_sb_cmp(&sb1, &sb4);
	TEST_ASSERT_EQ(r3, 1,
		"Comparison with same string (different case) succeed"TEST_EXPECT_MSG(d),
		1, r3);
	...
	TEST_ASSERT_EQ(cbuild_sb_cmp_icase(&sb5, &sb5), 0,
		"Comparison with self failed"TEST_EXPECT_MSG(d), 0,
		cbuild_sb_cmp_icase(&sb5, &sb5));
	int r8 = cbuild_sb_cmp_icase(&sb1, &sb2);
	TEST_ASSERT_EQ(r8, 0,
		"Case-less comparison with same string builder failed"
		TEST_EXPECT_MSG(d), 0, r8);
	int r9 = cbuild_sb_cmp_icase(&sb1, &sb3);
	TEST_ASSERT_EQ(r9, 0,
		"Case-less comparison with same string builder (different case) failed"
		TEST_EXPECT_MSG(d), 0, r8);
