int main(void) {
	...
	// Do testing
	TEST_ASSERT(cbuild_file_rename(tsrc, tdst),
		"cbuild_file_rename returned error");
	TEST_NASSERT(cbuild_file_check(tsrc),
		"Source file still exists after a rename.");
	TEST_ASSERT(cbuild_file_check(tdst),
		"Destination file does not exist after a rename.");
