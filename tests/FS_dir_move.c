int main(void) {
	...
	cbuild_fd_close(pattern_fd);
	// Do testing
	TEST_ASSERT(cbuild_dir_move(src, dst), "cbuild_dir_copy returned error.");
	...
	TEST_NASSERT(cbuild_file_check(cbuild_temp_sprintf("%s/a", src)),
		"File \"a\" exist after copy");
	TEST_NASSERT(cbuild_file_check(cbuild_temp_sprintf("%s/b", src)),
		"File \"b\" exist after copy");
	TEST_NASSERT(cbuild_dir_check(cbuild_temp_sprintf("%s/c", src)),
