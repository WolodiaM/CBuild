...
//       args->clear();
//       // Load init task
//       args->push_back("task_id", "CBuild_version");
//       return CBuild::TASK;
//       // Get help message
...
void CBuild::loop(CBuild::RType                       mode,
    ...
    case CBuild::BUILD: {
        if (*(args->get("toolchain_id")) == std::string("all")) {
            CBuild::Registry::ToolchainAll(force, *(args->get("curr_path")),
                                           &pargs);
        } else {
            // Load toolchain
...
mode_end:
    ...
    //     if (mode == CBuild::RUN || mode == CBuild::TASK || mode ==
    //     CBuild::CLEAR ||
...
pseudo:
    ...
    {
        CBuild::print_full("Using std::random_device for seed",
                           CBuild::MAGENTA);
