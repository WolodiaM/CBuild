...
CBuild::types::file_content get_file_data(std::string path, std::string included_from = "") {
    ...
    while (std::regex_search(start, str.cend(), match, CBuild::consts::include_parser)) {
        // Get file
...
void CBuild::print_files() {
    CBuild::print_full("\t\t\tCBuild hash v3.0 - Filelist", CBuild::color::MAGENTA);
    std::stringstream buff;
...
std::vector<std::string> CBuild::get_files(std::vector<std::string> files,
    ...
    // Clear oll things
    CBuild::vars::headers.clear();
    CBuild::vars::filelist.clear();
    CBuild::vars::old_hashes.clear();
    // Return value
    return ret;
}
