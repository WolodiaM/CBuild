...
void CBuild::Toolchain::depends_on_project(std::string path, std::string name, std::string id,
...
void CBuild::Toolchain::set_profiling_mode() {
    this->add_compile_arg("-pg");
    this->add_link_arg("-pg");
}
...
lib::map<std::string, std::string> CBuild::Toolchain::gen_file_list_force() {
    bool tmp = this->force;
    this->force = true;
    auto ret = this->gen_file_list(true);
    this->force = tmp;
    return ret;
}
std::string CBuild::Toolchain::get_compile_args() {
    std::string ret = "";
    for (auto arg : this->compiler_args) {
        ret += arg;
        ret += " ";
    }
    return ret;
}
std::string CBuild::Toolchain::gen_out_name(std::string executable, std::string dyn_lib,
                                            std::string stat_lib) {
    // Base path
    std::string base = "";
    if (!this->gen_out_name_without_base_path) {
        base = CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_OUT_DIR + "/";
    }
    // If name defined
...
void CBuild::Toolchain::init() {
    ...
    if (!CBuild::fs::exists(CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_CACHE_DIR)) {
        CBuild::fs::create({CBUILD_BUILD_DIR + "/" + this->id + "/" + CBUILD_BUILD_CACHE_DIR},
                           CBuild::fs::DIR);
    }
...
void CBuild::Toolchain::load_project_deps(std::string curr_path __attribute_maybe_unused__) {
    for (auto elem : this->project_deps) {
        ...
        CBuild::system(cmd);
        // Copy files
        CBuild::system("cp -r " + elem.path + "/" + CBUILD_BUILD_DIR + "/" + elem.id + "/" +
                       CBUILD_BUILD_OUT_DIR + "/*.* " + CBUILD_CACHE_DIR + "/" +
...
std::array<std::string, 3> CBuild::Toolchain::get_cmds() {
    return std::array<std::string, 3>{this->compiler, this->linker, this->packer};
}
void CBuild::Toolchain::crash() {
    CBuild::print("Error in compilation! Stopping compilation...", CBuild::RED);
    exit(0xFF);
}
void CBuild::Toolchain::compile(std::string str) {
...
void CBuild::Toolchain::call(std::vector<std::string>* args, bool force, bool debug, bool dummy) {
    ...
    // 5-th step - special linking for static libraries
    ...
    this->post_link();
    if (this->build_type == CBuild::DYNAMIC_LIBRARY && this->version_major != -1) {
        ...
        this->gen_out_name_without_base_path = true;
        CBuild::system("cd " + CBUILD_BUILD_DIR + "/" + this->get_id() + "/" +
